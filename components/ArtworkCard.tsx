    ...
    <div className="group relative">
      <Link href={artworkHref(art)} className="block">
        <div className="relative aspect-[4/5] overflow-hidden border border-stone bg-ivory">
          ...
          />
          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/75 to-transparent p-3 opacity-0 transition-opacity duration-500 ease-gallery group-hover:opacity-100">
            <p className="truncate text-xs text-white/90">{art.artist}</p>
            <p className="truncate font-editorial text-sm text-white">{art.title}</p>
          </div>
          {SOURCE_LABEL[art.source] && (
            <span className="absolute left-2 top-2 rounded-full bg-paper/90 px-2 py-0.5 text-[0.65rem] uppercase tracking-label text-ink-soft backdrop-blur md:text-xs">
              {SOURCE_LABEL[art.source]}
            </span>
          )}
          {fitsFrame(art.aspect) && (
            <span
              className="absolute right-2 top-2 rounded-full bg-brass px-2 py-0.5 text-[0.65rem] uppercase tracking-label text-paper backdrop-blur md:text-xs"
              title="Crops cleanly to your Frame's 16:9"
              aria-label="Fits your Frame — crops cleanly to 16:9"
            >
              16:9
            </span>
          )}
